"""Fixed-protocol supplementary analysis of public EV capacity proxies."""
import argparse
import hashlib
import json
import platform
...
def run(source, output, models_dir):
    ...
    train = data.loc[data.split == 'train']
    metrics, predictions, cv_metrics, fold_manifest = [], [], [], []
    primary = {}
    ...
    def evaluate(model, name, columns, seed=42, stress=None):
        for split in ['validation', 'test']:
            ...
            records = part[['vehicle', 'split', 'start_local_unspecified', 'proxy_capacity_ah']].copy()
    ...
    code_files = [Path(__file__), Path(__file__).with_name('ev_experiment.py'), Path(__file__).with_name('V2_PROTOCOL.md')]
    model_files = list(models_dir.glob('*.joblib'))
    manifest = {
        'base_commit': subprocess.check_output(['git', 'rev-parse', 'HEAD'], text=True).strip(),
        'label': 'derived capacity proxy in Ah; not independently measured SOH',
        'input_file': 'results/ev/session_features.csv', 'input_sha256': sha256(source),
        'code_sha256': {p.name: sha256(p) for p in code_files},
        'models_sha256': {p.name: sha256(p) for p in model_files},
        'raw_extraction_rerun': False,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=Path, default=Path('research/public_battery/results/ev/session_features.csv'))
    parser.add_argument('--output', type=Path, default=Path('research/public_battery/results/v2'))
